...
FC 時代のウィザードリィを FC 時代の FF 風の UI にアレンジした二次創作ゲームです。レトロゲームエンジン [Pyxel](https://github.com/kitao/pyxel) を使って制作しています。
...
Linux上のブラウザではパッドが効かない・音が全面的に割れる・画面サイズが極端に小さくなるなど正常に動作しないことが多いため、ぜひこちらをお試しください。
...
2026/02/03 マップ移動処理高速化（ハンドヘルド機などの低スペックマシンでも快適に動作するように）
...
2024/04/01 Pyxelパッケージファイル(.pyxapp)追加 ※ゲーム内容に変更はないため、ゲーム内バージョン表記は2023/12/10のまま

2023/12/10 Pyxel2.0 版に変更（ゲーム自体に変更はないが、環境により BGM のノイズが発生する事象が改善）、フロア移動時のフェードアニメーション有効化

2023/12/05 サーバーセーブ・ロードに対応、クリア後の商店で未販売アイテムの表示を追加
